...
class DataSourceType(models.Model):
    ...
    name = models.CharField(
        max_length=100, help_text='Name of the data source type',
    )
...
class DataSource(models.Model):
    ...
    type = models.ForeignKey(
        DataSourceType, on_delete=models.DO_NOTHING, help_text='Type of the data source',
    )
    owner = models.ForeignKey(
        User, on_delete=models.DO_NOTHING, help_text='Owner of the data source',
    )
    name = models.CharField(
        max_length=100, help_text='Name of the data source',
    )
    size = models.IntegerField(
        default=0, help_text='Size of the data source in bytes',
    )
...
class DataSourceEntry(models.Model):
    ...
    """
    uuid = models.UUIDField(
        default=uuid.uuid4, editable=False, help_text='UUID of the data source file',
    )
    datasource = models.ForeignKey(
        DataSource, on_delete=models.DO_NOTHING, help_text='Data source',
    )
    config = models.JSONField(
        default=dict, help_text='Config for the data source entry',
    )
    name = models.CharField(
        max_length=2048, help_text='Name of the data source file', blank=True, default='',
    )
    size = models.IntegerField(
        default=0, help_text='Size of the entry in bytes',
    )
    status = models.CharField(
        default=DataSourceEntryStatus.PROCESSING, max_length=100,
        choices=DataSourceEntryStatus.choices, help_text='Status of the data source entry',
    )
    refresh_frequency = models.CharField(
        default=RefreshFreqeuncy.ONCE, max_length=100,
        choices=RefreshFreqeuncy.choices, help_text='Refresh frequency of the data source entry',
    )
    created_at = models.DateTimeField(
        help_text='Time when the data source file was created', default=now,
    )
    updated_at = models.DateTimeField(
        auto_now=True, help_text='Time when the data source file was updated',
    )
